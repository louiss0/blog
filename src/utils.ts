...
import {
  COLLECTION_NAMES,
  type AllCollectionNames,
  type AllCollectionNamesValues,
} from "./collections";
...
export const aWordWithOnlyLettersAndStartsWithUppercase = (value: string) =>
  /^(?:[A-Z][a-z]*)$/.test(value);

export const isCapitalized = (value: string) =>
  value.split(" ").every(aWordWithOnlyLettersAndStartsWithUppercase);
...
export const curryFunctionToCallWithOppositeBoolean =
  ...
  (...args: Array<NonNullable<unknown>>) =>
    !cb(...args);
...
interface GetAllCollections {
  ...
  ): Promise<Array<CollectionEntry<AllCollectionNamesValues>>>;
}

export const getAllCollections: GetAllCollections = async <
  U extends CollectionEntry<AllCollectionNamesValues>,
>(
...
export const attemptToCreateSpacedStringFromLowerCaseAndDashedFileNameByCapturingTheFilenameAndExtension =
  (fileName: string) => {
    const fileNameAndExtensionMatchGroups = fileName.match(
      /(?<file_name>[\w-]+)(?<extension>\.[a-z]+)/,
    )?.groups;
...
export const buildTableOfContentsData = (
  ...
  return headings.reduce(
    (
      headingsWithSubHeadingsList: Array<MarkdownHeadingWithSubHeadings>,
      heading,
    ) => {
