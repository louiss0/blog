...
export const CATEGORIES = z.enum(["Segment", "Tutorial", "Standalone"]);

export type Category = z.infer<typeof CATEGORIES>;

const categoryWithPartAndSeriesSchema = z.object({
  category: z.literal(CATEGORIES.Values.Segment),
  series: z
    .string({
      required_error:
        "A post that is a segment must have a series associated with it",
    })
    .max(45)
    .regex(
      /\b[A-Z][A-Za-z0-9\s]*\b/,
  ...
  part: z
    .number({
      required_error:
        "A post with a segment must have a part number associated with it",
    })
    .int(),
});
...
const categoryWithoutPartAndSeriesSchema = z.object({
  category: z
    .literal(CATEGORIES.Values.Standalone)
    .or(z.literal(CATEGORIES.Values.Tutorial)),
  part: z.undefined(),
  series: z.undefined(),
});
...
const seriesSchema = categoryWithPartAndSeriesSchema.or(
  categoryWithoutPartAndSeriesSchema,
);

export const postSchema = baseSchema.and(seriesSchema).and(
  z.object({
    ...
    ({ image }: SchemaContext) =>
      postSchema
        .and(
          z.object({
            heroImage: image(),
            tags: z.array(z.string()).refine(getArrayStartsWithString(string)),
          }),
        )
...
export const collections = {
  astro: defineCollection({
    schema:
      getCreatePostSchemaBasedOnIfATagStartsWithPassedInValueAndImageFromSchemaContext(
        "astro",
      ),
  }),
...
export const COLLECTION_NAMES = Object.keys(collections) as ReadonlyArray<
